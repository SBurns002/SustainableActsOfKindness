/*
  # Fix Event ID References

  1. Database Changes
    - Update event_participants.event_id to reference events.id as UUID
    - Update event_reminders.event_id to reference events.id as UUID
    - Update event_notifications.event_id to reference events.id as UUID
    - Add proper foreign key constraints

  2. Data Migration
    - Migrate existing text-based event_id values to proper UUID references
    - Handle cases where events don't exist in the events table

  3. Security
    - Update RLS policies to work with new UUID references
*/

-- First, let's add a temporary column to store the new UUID references
...
DROP INDEX IF EXISTS event_participants_user_id_event_id_key;
DROP INDEX IF EXISTS event_reminders_user_event_unique;
