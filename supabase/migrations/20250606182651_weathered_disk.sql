/*
  # Comprehensive MFA Cleanup and Reset

  This migration performs a complete cleanup of MFA-related data for troubleshooting
  and ensures a clean state for MFA functionality.

  ## What this does:
  1. Removes all existing MFA factors and challenges for sdavies8@bu.edu
  2. Resets session authentication levels
  3. Cleans up any orphaned or expired MFA data globally
  4. Provides detailed logging for debugging

  ## Security Note:
  This is a troubleshooting migration that removes MFA protection temporarily.
  The user should re-enable MFA after this cleanup.
*/
...
BEGIN
  ...
  IF target_user_id IS NULL THEN
    RETURN QUERY SELECT 'user_lookup'::text, 'User not found: ' || user_email, false;
    RETURN;
  END IF;
  
  RETURN QUERY SELECT 'user_found'::text, 'User ID: ' || target_user_id::text, true;
  ...
  RETURN QUERY SELECT 'initial_count'::text, 
    format('Factors: %s, Challenges: %s, Sessions: %s', factor_count, challenge_count, session_count), 
    true;
  ...
  BEGIN
    ...
    RETURN QUERY SELECT 'challenges_removed'::text, 'Removed ' || challenge_count || ' challenges', true;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN QUERY SELECT 'challenges_error'::text, 'Error: ' || SQLERRM, false;
  END;
  ...
  BEGIN
    ...
    RETURN QUERY SELECT 'factors_removed'::text, 'Removed ' || factor_count || ' factors', true;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN QUERY SELECT 'factors_error'::text, 'Error: ' || SQLERRM, false;
  END;
  ...
  BEGIN
    ...
    RETURN QUERY SELECT 'sessions_reset'::text, 'Reset ' || session_count || ' sessions to aal1', true;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN QUERY SELECT 'sessions_error'::text, 'Error: ' || SQLERRM, false;
  END;
  ...
  BEGIN
    ...
    RETURN QUERY SELECT 'user_updated'::text, 'User record timestamp updated', true;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN QUERY SELECT 'user_update_error'::text, 'Error: ' || SQLERRM, false;
  END;
  ...
  RETURN QUERY SELECT 'final_verification'::text, 
    format('Remaining - Factors: %s, Challenges: %s', factor_count, challenge_count), 
...
-- Add helpful comment
COMMENT ON SCHEMA auth IS 'Supabase auth schema - MFA cleanup completed on ' || now()::text;